...
public class QuietTimeGui extends GuiScreen {
    ...
    public void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 2:
                ...
                break;
            case 3:
                settings.setQuietTimeConfig(QuietTimeConfig.NORMAL);
                this.settings.saveSettings();
                setTextBoxContent();
                break;
            case 4:
                settings.setQuietTimeConfig(QuietTimeConfig.GJUM_SPECIAL);
                this.settings.saveSettings();
                setTextBoxContent();
                break;
            case 5:
                settings.setQuietTimeConfig(QuietTimeConfig.HIDE_COORDS);
                this.settings.saveSettings();
                setTextBoxContent();
                break;
            case 6:
                settings.setQuietTimeConfig(QuietTimeConfig.HIDE_COORDS_AND_NAME);
                this.settings.saveSettings();
                setTextBoxContent();
                break;
        }
    ...
    private void setTextBoxContent() {
        final QuietTimeConfig config = settings.getQuietTimeConfig();
